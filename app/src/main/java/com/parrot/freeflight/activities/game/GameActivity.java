        ...
        DroneBatteryChangedReceiverDelegate, DroneFlyingStateReceiverDelegate, DroneCameraReadyActionReceiverDelegate, DroneRecordReadyActionReceiverDelegate, SettingsDialogDelegate
{
    ...
    private GameJoystickListener rollPitchListener;
    private GameJoystickListener gazYawListener;

    private GameHudViewController view;
...
//            forceCombinedControlMode = false;
    ...
    {
        rollPitchListener = new GameJoystickListener()
        {

            public void onChanged(GameJoystickBase joy, float x, float y)
            {
...
//                    droneControlService.setPitch(-y);
            ...
            @Override
            public void onPressed(GameJoystickBase joy)
            {
//                leftJoyPressed = true;
//
...
//                running = true;
            ...
            @Override
            public void onReleased(GameJoystickBase joy)
            {
...
//                running = false;
        ...
        gazYawListener = new GameJoystickListener()
        {
...
//                gameController.startTrackball();   //换为跟踪小球
    ...
    private void initVirtualJoysticks(GameHudViewController.JoystickType leftType, GameHudViewController.JoystickType rightType, boolean isLeftHanded)
    {
        GameJoystickBase joystickLeft = (!isLeftHanded ? view.getJoystickLeft() : view.getJoystickRight());
        GameJoystickBase joystickRight = (!isLeftHanded ? view.getJoystickRight() : view.getJoystickLeft());

        ApplicationSettings settings = getSettings();

        if (leftType == GameHudViewController.JoystickType.ANALOGUE) {
            if (joystickLeft == null || !(joystickLeft instanceof GameAnalogueJoystick) || joystickLeft.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickLeft = GameJoystickFactory.createAnalogueJoystick(this, settings.isAbsoluteControlEnabled(), rollPitchListener);
            } else {
                joystickLeft.setOnAnalogueChangedListener(rollPitchListener);
                joystickRight.setAbsolute(settings.isAbsoluteControlEnabled());
            }
        } else if (leftType == GameHudViewController.JoystickType.ACCELERO) {
            if (joystickLeft == null || !(joystickLeft instanceof GameAcceleroJoystick) || joystickLeft.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickLeft = GameJoystickFactory.createAcceleroJoystick(this, settings.isAbsoluteControlEnabled(), rollPitchListener);
            } else {
                joystickLeft.setOnAnalogueChangedListener(rollPitchListener);
                joystickRight.setAbsolute(settings.isAbsoluteControlEnabled());
            }
        ...
        if (rightType == GameHudViewController.JoystickType.ANALOGUE) {
            if (joystickRight == null || !(joystickRight instanceof GameAnalogueJoystick) || joystickRight.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickRight = GameJoystickFactory.createAnalogueJoystick(this, false, gazYawListener);
            } else {
                joystickRight.setOnAnalogueChangedListener(gazYawListener);
                joystickRight.setAbsolute(false);
            }
        } else if (rightType == GameHudViewController.JoystickType.ACCELERO) {
            if (joystickRight == null || !(joystickRight instanceof GameAcceleroJoystick) || joystickRight.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickRight = GameJoystickFactory.createAcceleroJoystick(this, false, gazYawListener);
            } else {
                joystickRight.setOnAnalogueChangedListener(gazYawListener);
                joystickRight.setAbsolute(false);
            }
...
//        gameController.stop();
    ...
    private void applyJoypadConfig(ApplicationSettings.ControlMode controlMode, boolean isLeftHanded)
    {
        switch (controlMode) {
            case NORMAL_MODE:
                initVirtualJoysticks(GameHudViewController.JoystickType.ANALOGUE, GameHudViewController.JoystickType.ANALOGUE, isLeftHanded);
                acceleroEnabled = false;
                break;
            case ACCELERO_MODE:
                initVirtualJoysticks(GameHudViewController.JoystickType.ACCELERO, GameHudViewController.JoystickType.ANALOGUE, isLeftHanded);
                acceleroEnabled = true;
                break;
            case ACE_MODE:
                initVirtualJoysticks(GameHudViewController.JoystickType.NONE, GameHudViewController.JoystickType.COMBINED, isLeftHanded);
                acceleroEnabled = true;
                break;
        }
    }
