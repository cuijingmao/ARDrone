...
public class FirmHudViewController
    ...
    private FirmVideoStageView canvasView;
    ...
    public FirmHudViewController(Activity context, boolean useSoftwareRendering) {
        joypadOpacity = 1f;
         ...
         * 此处  控制了FirmActivity的界面
         */
        renderer = new FirmVideoStageRenderer(context, null);  //render 执行真正的渲染工作
...
//			root.removeView(glView);
            ...
            canvasView = new FirmVideoStageView(context);
...
//		renderer.addSprite(USB_INDICATOR_TEXT_ID, txtUsbRemaining);
    ...
    private void initGLSurfaceView() {
        if (glView != null) {
            /**
             * GLSurfaceView中需要setRenderer(renderer)来设置渲染器，这是它最主要的方法
             * render才是渲染绘图的主体
             */
