...
public class TaskHudViewController
...
//	private TextView txtVideoFps;
	...
	private TaskJoystickBase[] joysticks;
	...
	public TaskHudViewController(Activity context, boolean useSoftwareRendering)
	{
	    joypadOpacity = 1f;
		...
		canvasView = null;
		joysticks = new TaskJoystickBase[2];
...
//
	...
	public void setJoysticks(TaskJoystickBase left, TaskJoystickBase right)
	{
		joysticks[0] = left;
		if (left != null)   {
		...
		for (int i=0; i<joysticks.length; ++i) {
		    TaskJoystickBase joystick = joysticks[i];
	...
	public TaskJoystickBase getJoystickLeft()
	{
	    return joysticks[0];
	}
	
	
	public TaskJoystickBase getJoystickRight()
	{
	    return joysticks[1];
	}
	...
	public boolean onTouch(View v, MotionEvent event)
	{
		...
		if (result != true) {	
			...
			for (int i=0; i<joysticks.length; ++i) {
				TaskJoystickBase joy = joysticks[i];
				if (joy != null) {
