        ...
        DroneBatteryChangedReceiverDelegate, DroneFlyingStateReceiverDelegate, DroneCameraReadyActionReceiverDelegate, DroneRecordReadyActionReceiverDelegate, SettingsDialogDelegate
{
    ...
    private BallJoystickListener rollPitchListener;
    private BallJoystickListener gazYawListener;

    private BallHudViewController view;
    ...
    private ServiceConnection mConnection = new ServiceConnection()
    {
        ...
        public void onServiceConnected(ComponentName name, IBinder service)
        {
             ...
             */
            gameController = new BallController(BallActivity.this, droneControlService);
...
//            forceCombinedControlMode = false;
        ...
        view = new BallHudViewController(this, useSoftwareRendering);
    ...
    {
        rollPitchListener = new BallJoystickListener()
        {

            public void onChanged(BallJoystickBase joy, float x, float y)
            {
...
//                    droneControlService.setPitch(-y);
            ...
            @Override
            public void onPressed(BallJoystickBase joy)
            {
//                leftJoyPressed = true;
//
...
//                running = true;
            ...
            @Override
            public void onReleased(BallJoystickBase joy)
            {
...
//                running = false;
        ...
        gazYawListener = new BallJoystickListener()
        {
...
//                gameController.startTrackball();   //换为跟踪小球
    ...
    private void initVirtualJoysticks(BallHudViewController.JoystickType leftType,BallHudViewController.JoystickType rightType, boolean isLeftHanded)
    {
        BallJoystickBase joystickLeft = (!isLeftHanded ? view.getJoystickLeft() : view.getJoystickRight());
        BallJoystickBase joystickRight = (!isLeftHanded ? view.getJoystickRight() : view.getJoystickLeft());

        ApplicationSettings settings = getSettings();

        if (leftType == BallHudViewController.JoystickType.ANALOGUE) {
            if (joystickLeft == null || !(joystickLeft instanceof BallAnalogueJoystick) || joystickLeft.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickLeft = BallJoystickFactory.createAnalogueJoystick(this, settings.isAbsoluteControlEnabled(), rollPitchListener);
            } else {
                joystickLeft.setOnAnalogueChangedListener(rollPitchListener);
                joystickRight.setAbsolute(settings.isAbsoluteControlEnabled());
            }
        } else if (leftType == BallHudViewController.JoystickType.ACCELERO) {
            if (joystickLeft == null || !(joystickLeft instanceof BallAcceleroJoystick) || joystickLeft.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickLeft = BallJoystickFactory.createAcceleroJoystick(this, settings.isAbsoluteControlEnabled(), rollPitchListener);
            } else {
                joystickLeft.setOnAnalogueChangedListener(rollPitchListener);
                joystickRight.setAbsolute(settings.isAbsoluteControlEnabled());
            }
        ...
        if (rightType == BallHudViewController.JoystickType.ANALOGUE) {
            if (joystickRight == null || !(joystickRight instanceof BallAnalogueJoystick) || joystickRight.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickRight = BallJoystickFactory.createAnalogueJoystick(this, false, gazYawListener);
            } else {
                joystickRight.setOnAnalogueChangedListener(gazYawListener);
                joystickRight.setAbsolute(false);
            }
        } else if (rightType == BallHudViewController.JoystickType.ACCELERO) {
            if (joystickRight == null || !(joystickRight instanceof BallAcceleroJoystick) || joystickRight.isAbsoluteControl() != settings.isAbsoluteControlEnabled()) {
                joystickRight = BallJoystickFactory.createAcceleroJoystick(this, false, gazYawListener);
            } else {
                joystickRight.setOnAnalogueChangedListener(gazYawListener);
                joystickRight.setAbsolute(false);
            }
...
//        gameController.stop();
    ...
    private void applyJoypadConfig(ApplicationSettings.ControlMode controlMode, boolean isLeftHanded)
    {
        switch (controlMode) {
            case NORMAL_MODE:
                initVirtualJoysticks(BallHudViewController.JoystickType.ANALOGUE, BallHudViewController.JoystickType.ANALOGUE, isLeftHanded);
                acceleroEnabled = false;
                break;
            case ACCELERO_MODE:
                initVirtualJoysticks(BallHudViewController.JoystickType.ACCELERO, BallHudViewController.JoystickType.ANALOGUE, isLeftHanded);
                acceleroEnabled = true;
                break;
            case ACE_MODE:
                initVirtualJoysticks(BallHudViewController.JoystickType.NONE, BallHudViewController.JoystickType.COMBINED, isLeftHanded);
                acceleroEnabled = true;
                break;
        }
    }
